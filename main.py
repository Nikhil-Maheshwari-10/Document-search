...
# Add checkbox for image processing option
...
# Add a button to delete the Qdrant collection (moved below uploader)
if st.button("Clear Storage"):
    try:
        ...
        st.stop()  
        st.rerun()  # Force rerun to clear any UI showing previously uploaded files
    except Exception as e:
...
if uploaded_files and not st.session_state.upload_complete:
    ...
    total_processing_time = 0  # Track total time
    for idx, uploaded_file in enumerate(uploaded_files):
        ...
        image_descriptions = []
        if process_images and uploaded_file.name.lower().endswith("pdf"):
            try:
            ...
            logger.info("Image processing skipped as per user selection.")
        ...
        total_processing_time += elapsed  # Add to total
        logger.info(f"Processing time for {uploaded_file.name}: {elapsed:.2f} seconds")
    # After all files processed, print total time
    logger.info(f"Total processing time for {len(uploaded_files)} file(s): {total_processing_time:.2f} seconds")
